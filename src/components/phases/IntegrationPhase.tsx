// src/components/phases/IntegrationPhase.tsx
...
const mockInitialScores = [
  {
    id: 1,
    essential_number: 1,
    essential_name: 'Organize for disaster resilience',
    score: 65,
    notes: 'Strong DRRM committee but limited stakeholder engagement',
  },
  {
    id: 2,
    essential_number: 2,
    essential_name: 'Identify, understand and use current and future risk scenarios',
    score: 72,
    notes: 'Comprehensive risk assessments completed annually',
  },
  {
    id: 3,
    essential_number: 3,
    essential_name: 'Strengthen financial capacity for resilience',
    score: 45,
    notes: 'Limited budget allocation, seeking external funding',
  },
  {
    id: 4,
    essential_number: 4,
    essential_name: 'Pursue resilient urban development and design',
    score: 58,
    notes: 'Building code enforcement needs improvement',
  },
  {
    id: 5,
    essential_number: 5,
    essential_name: 'Safeguard natural buffers to help mitigate hazards',
    score: 82,
    notes: 'Excellent mangrove conservation programs',
  },
  {
    id: 6,
    essential_number: 6,
    essential_name: 'Strengthen institutional capacity for resilience',
    score: 53,
    notes: 'Staff training programs need expansion',
  },
  {
    id: 7,
    essential_number: 7,
    essential_name: 'Understand and strengthen societal resilience',
    score: 48,
    notes: 'Community engagement requires more inclusive approaches',
  },
  {
    id: 8,
    essential_number: 8,
    essential_name: 'Increase infrastructure resilience',
    score: 61,
    notes: 'Critical infrastructure assessment completed, retrofitting ongoing',
  },
  {
    id: 9,
    essential_number: 9,
    essential_name: 'Ensure effective preparedness and response',
    score: 75,
    notes: 'Well-developed emergency response plans with regular testing',
  },
  {
    id: 10,
    essential_number: 10,
    essential_name: 'Recover better after disasters',
    score: 42,
    notes: 'Recovery frameworks need strengthening',
  },
];
...
export const IntegrationPhase: React.FC = () => {
  ...
  const [historicalData] = useState([
    {
      year: 2022,
      overallScore: 52.3,
      essentials: mockInitialScores.map((s) => ({ ...s, score: s.score - 8 })),
    },
    {
      year: 2023,
      overallScore: 58.7,
      essentials: mockInitialScores.map((s) => ({ ...s, score: s.score - 4 })),
    },
    { year: 2024, overallScore: 60.1, essentials: mockInitialScores },
  ]);

  const calculateOverallScore = (currentScores: ScoreEntry[]) => {
    if (currentScores.length === 0) return 0;

    const organizationalEssentials = [1, 6];
    const riskAssessmentEssentials = [2];
    const financialEssentials = [3];
    const infrastructureEssentials = [4, 5, 8];
    const communityEssentials = [7, 9, 10];

    const getAverageScore = (essentialNumbers: number[]) => {
      const relevantScores = currentScores.filter((s) =>
        essentialNumbers.includes(s.essential_number)
      );
      return relevantScores.length > 0
        ? relevantScores.reduce((sum, s) => sum + s.score, 0) /
  ...
  const calculateEssentialScore = (indicators: IndicatorEntry[]): number => {
  ...
  const handleAssessmentSubmit = () => {
    ...
    const essentialInfo = essentials.find(
      (e) => e.number === selectedEssential.number
    );
      ...
      score.essential_number === selectedEssential.number
        ? {
            ...
            notes:
              ...
              `Assessment based on ${assessmentData.indicators.filter(
                (ind) => ind.rating
              ).length} indicators`,
          }
        : score
  ...
  const updateIndicatorRating = (index: number, rating: number) => {
  ...
  const updateIndicatorEvidence = (index: number, evidence: string) => {
    const newIndicators = [...assessmentData.indicators];
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {scores.length > 0
                  ? Math.max(...scores.map((s) => s.score)).toFixed(0)
                  : 0}
                %
              </p>
            </div>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              Robust Resilience Scoring Methodology
            </h3>
            <p className="text-sm text-gray-700 mb-2">
              Our resilience score uses a weighted formula based on UNDRR
              guidelines and Philippine DRRM priorities:
            </p>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex-1">
            <div className="w-full bg-gray-200 rounded-full h-8 overflow-hidden">
              <div
                className={`h-full transition-all duration-1000 flex items-center justify-end pr-3 ${getScoreColor(
                  overallScore
                )}`}
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            UNDRR 10 Essentials Scorecard
          </h3>
          <p className="text-sm text-gray-600 mt-1">
            Track progress across all essentials for disaster resilience
          </p>
        </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-700">
                Your resilience score has improved by{' '}
                {previousYearScore > 0
                  ? ((yearOverYearChange / previousYearScore) * 100).toFixed(1)
                  : '0.0'}
                % compared to last year. Focus areas for continued improvement
                include Financial Capacity (Essential 3) and Community
                Resilience (Essentials 7 & 10).
              </p>
            </div>
          </div>
        </div>
            ...
            {selectedEssential ? (
              <div className="space-y-6">
                      ...
                      <div key={index} className="border rounded-lg p-4">
                          ...
                          <div className="flex space-x-2">
                            {[1, 2, 3, 4, 5].map((rating) => (
                              <button
                                key={rating}
                                onClick={() =>
                                  updateIndicatorRating(index, rating)
                                }
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              updateIndicatorEvidence(index, e.target.value)
                            }
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Rating Scale: 1=Inadequate, 2=Developing, 3=Satisfactory,
              ...
              <div className="space-y-4">
                <p className="text-gray-700">
                  Select an essential to conduct a detailed resilience
                  assessment. Each essential is evaluated based on key
                  indicators using a 1-5 rating scale.
                </p>
